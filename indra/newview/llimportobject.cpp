	...
	virtual bool tick()
	{
		...
		for(std::vector<LLUUID>::iterator itr = mRoots.begin();itr != mRoots.end();itr++)
		{
			...
			if(object)
			{
				...
				{
					LL_INFOS() << "WAIT " << (*itr).asString() << LL_ENDL;
...
void LLXmlImport::onNewPrim(LLViewerObject* object)
{
	...
	{
		LL_WARNS() << "called onNewPrim without getting back to IMPORT_INIT" << LL_ENDL;
		return;
	}
...
void LLXmlImport::onUpdatePrim(LLViewerObject* object)
{
	...
	default:
		{
			LL_WARNS() << "got default state in state machine, resetting to IMPORT_INIT" << LL_ENDL;
			sState = IMPORT_INIT;
		}
	}
	LLSelectMgr::getInstance()->deselectAll();
